...
import { Token, Pair, BigintIsh, BestTradeOptions, TokenAmount, Trade } from '@uniswap/sdk';
import { BigNumber, BigNumberish } from 'ethereum/utils/balancer-math';

export type { BigNumber, BigintIsh, Token, Pair, BestTradeOptions, TokenAmount, Trade };
 ...
 */
export const toUniswapSDKTokens = (chainID: number, tokenInputs: TokenInput[]): Token[] => tokenInputs.map(
  (token) => new Token(
    chainID,
 ...
 */
export const bigIntIshToBigNumber = (amount: BigintIsh): BigNumber => new BigNumber(amount.toString(16), 16);
 ...
 */
export const bigNumberishToBigInt = (amount: BigNumberish): BigInt => BigInt(new BigNumber(amount.toString(16), 16).toString());
