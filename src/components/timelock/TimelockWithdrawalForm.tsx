...
export function TimelockWithdrawalFormInner({
  lock,
  values,
  setFieldValue,
}: Props) {
  ...
  const sufficientDndx = useMemo(
    () => burnAmount.exact.lte(dndxBalance.exact),
    [burnAmount.exact, dndxBalance.exact]
  );
  ...
  const earlyWithdrawalFee = isReady
    ? undefined
    : calculateEarlyWithdrawalFee(
        values.amount.exact,
        lock.unlockAt,
  ...
  const earlyWithdrawalFeePercent = isReady
    ? undefined
    : calculateEarlyWithdrawalFeePercent(lock.unlockAt, lock.duration);
            ...
            <>
              ...
              >
                <span style={{ color: "#A61C23" }}>
                  {earlyWithdrawalFee.displayed} NDX (
                  {earlyWithdrawalFeePercent}
                  %)
                </span>
                <br />
                <Switch
                  checked={didAcknowledgeFee}
                  onChange={setDidAcknowledgeFee}
                />{" "}
              ...
              >
                You will receive{" "}
                <span style={{ color: "#38ee7b" }}>
                  {convert.toBalance(
                    values.amount.exact.minus(earlyWithdrawalFee.exact),
                    18,
                    false,
                    4
                  )}
          ...
          description={
            ...
            >
              {sufficientDndx ? (
                ...
                <>
                  {" "}
                  This withdrawal requires that you burn {
                    burnAmount.displayed
                  }{" "}
                  dNDX, but you only have {dndxBalance.displayed}.
                </>
              )}
            </Typography.Title>
          }
