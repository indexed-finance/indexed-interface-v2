...
import useUniswapTradingPairs from "hooks/useUniswapTradingPairs";
...
export default function TradeInteraction({ pool }: Props) {
  ...
  const handleChange = useCallback((values: InteractionValues) => {
    ...
    if (lastTouchedField === "from") {
      ...
      const amountIn = convert.toToken(fromAmount.toString(), inputToken.decimals).toString(10);
      ...
      const amountOut = convert.toToken(toAmount.toString(), outputToken.decimals).toString(10);
      const bestTrade = calculateBestTradeForExactOutput(
        inputToken,
        outputToken,
        amountOut
      );
      values.fromAmount = parseFloat(bestTrade?.inputAmount.toFixed(4) ?? "0");
  }
  ...
  const handleSubmit = useCallback(({
    ...
    if (
      ...
      if (lastTouchedField === "from") {
        const amountIn = convert.toToken(fromAmount.toString(), inputToken.decimals).toString(10);
        trade = calculateBestTradeForExactInput(inputToken, outputToken, amountIn);
      } else {
        const amountOut = convert.toToken(toAmount.toString(), outputToken.decimals).toString(10);
  ...
  }, [dispatch, tokenLookup, calculateBestTradeForExactInput, calculateBestTradeForExactOutput]);
