...
export function formatAmount(exact: string | BigNumber, decimals: number = DEFAULT_DECIMAL_COUNT, precision?: number): Amount {
  return {
...
export function formatDividendsLock(lock: DividendsLock): FormattedDividendsLock {
  ...
  const earlyWithdrawalFee: number | BigNumber = unlocked ? 0 : calculateEarlyWithdrawalFee(amount.exact, lock.unlockAt, lock.duration).exact;
...
export function calculateEarlyWithdrawalFee(
  ...
  const dynamicFee = amount.times(
    baseEarlyWithdrawalFee.times(timeRemaining).times(multiplier.exact)
  ).div(
    convert.toToken('1', 36).times(duration)
  );
...
export function calculateEarlyWithdrawalFeePercent(
  ...
  const multiplier = calculateMultiplier(duration);
  const dynamicFee = baseEarlyWithdrawalFee.times(timeRemaining).times(multiplier.exact).div(one.times(duration));
