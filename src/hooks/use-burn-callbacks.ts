...
export function useSingleTokenBurnCallbacks(poolId: string) {
  ...
  const calculateAmountIn = useCallback(
    (tokenOutSymbol: string, typedAmountOut: string) => {
      if (pool) {
        // console.log(`Enter calc in ${tokenOutSymbol} ${typ}`)
        const outputToken = pool.tokens.entities[
  ...
  const executeBurn = useCallback((
    ...
    if (specifiedField === "from") {
      ...
      if (result.error) throw Error(`Caught error calculating burn output: ${result.error}`);
      const minTokenAmountOut = downwardSlippage(result.tokenAmountOut as BigNumber, SLIPPAGE_RATE);
      dispatch(
      ...
      if (result.error) throw Error(`Caught error calculating burn input: ${result.error}`);
      const maxAmountIn = upwardSlippage(result.poolAmountIn as BigNumber, SLIPPAGE_RATE);
      dispatch(
